...
if(isset($_POST['update_status'])) {
 ...
 $current_status = $row1['applicant_status'];


  // Update the status in the database
  $query = "UPDATE applicant_perosnal_info SET company='$status' WHERE applicant_number='$applicant_number'";
  $query_run = mysqli_query($conn, $query);

  // Update the date_status_updated
  $timezone = new DateTimeZone('Asia/Manila');
  $date = new DateTime('now', $timezone);
  $date_updated = $date->format('Y-m-d h:i:s A');
  $queryTime = "UPDATE applicant_perosnal_info SET date_status_updated='$date_updated' WHERE applicant_number='$applicant_number'";
  $queryrun = mysqli_query($conn, $queryTime);
  
  // Audit trail log
  $user_name = $_SESSION['firstname'] . ' ' . $_SESSION['lastname'];
  $activity = "System Admin Updated applicant number <b>$applicant_number</b> status from <b> $current_status</b> to <b> $status </b> ";
