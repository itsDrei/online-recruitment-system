...
if(isset($_POST['job_update'])){
  ...
  $current_job_status= $row['job_status'];
   ...
   if ($job_title != $current_job_title || $job_desc != $current_job_desc || $job_status != $current_job_status) {
    $activity = "System Admin updated job vacancy";
    if ($job_title != $current_job_title) {
      $activity .= " titled from <b>$current_job_title</b> to <b>$job_title</b>";
    }
    if ($job_desc != $current_job_desc) {
      $activity .= " titled <b>$current_job_title</b> description from <b>$current_job_desc</b> with new description <b>$job_desc</b>";
    }
